...
BUILD		:=	build
SOURCES		:=	source source/menu
DATA		:=	data
INCLUDES	:=	include
ASSETS      :=  assets
GRAPHICS	:=	$(ASSETS)/textures
ROMFS		:=	romfs
GFXBUILD	:=	$(ROMFS)/gfx
#---------------------------------------------------------------------------------
APP_VER					:= 1056
APP_TITLE				:= Level256 Installer
APP_DESCRIPTION			:= Level256 Installer
APP_AUTHOR				:= Tekito_256
PRODUCT_CODE			:= CTR-HB-L256
UNIQUE_ID				:= 0x25602
...
LIBS	:=  -lcurl -lmbedtls -lmbedx509 -lmbedcrypto -lminizip -ljansson -lz -lctru -lm
